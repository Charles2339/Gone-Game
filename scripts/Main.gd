...
func _handle_tap():
	...
	if state == GameState.MENU or state == GameState.DEAD:
		_start_game()
...
func _start_game():
	...
	spawner.spawn_timer     = 1.80
	spawner.obstacle_ranges = []
	spawner.wave_cooldown   = 0.0
...
func _on_player_died():
	...
	# Small delay before showing game over so ragdoll plays out briefly
